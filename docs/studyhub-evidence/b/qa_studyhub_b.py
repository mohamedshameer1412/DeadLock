"""Real-browser + real-local-model test of StudyHub B: real uvicorn, real Edge, real Ollama. Production scrypt cost."""
import os
import shutil
import sqlite3
import subprocess
...
DB = os.path.join(OUT, "studyhub_b.db")
UPLOADS = os.path.join(OUT, "b_uploads")
...
server = subprocess.Popen([PY, "-m", "uvicorn", "studyhub.web.app:app", "--port", str(PORT), "--log-level", "warning"],
                          cwd=REPO, env=env, stdout=open(os.path.join(OUT, "studyhub_uvicorn_b.log"), "w"), stderr=subprocess.STDOUT)
...
try:
    ...
    shots = os.path.join(OUT, "shots_studyhub_b")
    os.makedirs(shots, exist_ok=True)
    node = subprocess.run(["node", os.path.join(HERE, "browser_b.mjs"), BASE, EDGE, os.path.join(OUT, "edge_profile_b"), shots,
    ...
    for row in db.execute("select id, status, requested, produced, rejected, tier, model from mcq_jobs"):
        print("   job:", row)
    print("   questions stored:", db.execute("select count(*) from mcq_items").fetchone()[0],
          " solver-checked:", db.execute("select count(*) from mcq_items where solver='agreed'").fetchone()[0])
    print("   answer positions:", [r[0] for r in db.execute("select answer_index from mcq_items order by id")])
    print("   options per question (must all be 4):", {len(__import__('json').loads(r[0])) for r in db.execute("select options from mcq_items")})
    print("   spine runs (mcq):", db.execute("select count(*) from runs where domain='mcq'").fetchone()[0])
finally:
    stop()
